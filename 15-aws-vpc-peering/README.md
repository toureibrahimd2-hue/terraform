...
```
┌─────────────────────────────────────┐       ┌─────────────────────────────────────┐
│     Primary VPC (eu-north-1)         │       │    Secondary VPC (eu-west-3)        │
...
1. **Two VPCs**:
   - Primary VPC in eu-north-1(10.0.0.0/16)
   - Secondary VPC in eu-west-3 (10.1.0.0/16)
...
```bash
# For us-east-1
aws ec2 create-key-pair --key-name vpc-peering-demo --region eu-north-1--query 'KeyMaterial' --output text > vpc-peering-demo.pem

# For eu-west-3
aws ec2 create-key-pair --key-name vpc-peering-demo --region eu-west-3 --query 'KeyMaterial' --output text > vpc-peering-demo-west.pem
